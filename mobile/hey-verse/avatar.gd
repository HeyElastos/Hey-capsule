...
# Chassis FINISHES — the whole-robot look. "gold" is the FIRST-BATCH "naked
# robot" entry pass every avatar mints as; later tiers / the editor unlock the
# others (and the painted-suit BODY_COLORS). body = chassis (chest/head/thighs/
# shoulders), detail = joints (arms/boots/pelvis/neck/antenna), spec = sheen.
const FINISHES := {
	"gold":     {"body": Color(1.00, 0.78, 0.32), "detail": Color(0.52, 0.36, 0.12), "spec": 1.0, "rim": 0.55},
	"classic":  {"body": Color(0.95, 0.95, 0.97), "detail": Color(0.10, 0.12, 0.17), "spec": 0.7, "rim": 0.45},
	"silver":   {"body": Color(0.86, 0.88, 0.92), "detail": Color(0.34, 0.37, 0.43), "spec": 1.0, "rim": 0.55},
	"obsidian": {"body": Color(0.16, 0.17, 0.22), "detail": Color(0.06, 0.06, 0.09), "spec": 1.0, "rim": 0.60},
}
const DEFAULT_FINISH := "gold"
...
static func derive_traits(did: String) -> Dictionary:
	var h := absi(hash(did))
	return {
		# FIRST BATCH: every freshly-minted avatar is the gold "naked robot" entry
		# pass. Painted-suit bodies + other finishes unlock in the editor (and
		# later mint tiers). Face/antenna/visor still vary per DID so no two gold
		# robots look identical.
...
var _chassis_mat: ShaderMaterial        # the resolved finish/body material (shoulders reuse it)
...
func rebuild(new_outfit: Dictionary) -> void:
	...
	if outfit.has("catalog_hat") and str(outfit.get("catalog_hat", "")) != "":
		set_catalog_hat(str(outfit["catalog_hat"]))
	elif outfit.has("hat"):
...
## Wear a real CATALOG headwear (VerseCatalog item, e.g. "crown", "wizard_hat")
## on the crown anchor. "" / "none" clears it. Catalog hats are modelled at the
## origin sized to drop onto _hat_root, so they mount as-is. This is the wear
## path a bought .ddrm hat will use later (build → mount), so the dress-up UI
## and the marketplace share one code path.
## Over-ear / side-of-head items: scaled to the head width and dropped to ear
## height instead of perching on the crown.
const EAR_ITEMS := ["headphones"]

func set_catalog_hat(id: String) -> void:
	if _hat_root == null:
		return
	for child in _hat_root.get_children():
		child.queue_free()
	if id == "" or id == "none":
		return
	var m: Node3D = VerseCatalog.build(id)
	if m == null:
		return
	if EAR_ITEMS.has(id):
		# the head is ~0.94 wide; the catalog item is ~0.3 — scale it up to wrap
		# the head SIDES and drop it from the crown (y≈1.83) to ear height (~1.49).
		m.scale = Vector3(2.7, 2.7, 2.7)
		m.position.y = -0.33
	_hat_root.add_child(m)


## Hide the floating name tag (e.g. while the close-up avatar editor is open so
## it doesn't sit over the robot's head).
...
func set_remote_state(pos: Vector3, yaw: float, is_moving: bool, is_sitting: bool = false) -> void:
	# Teleport (walking through a door / entering a house / world jump): SNAP rather
	# than glide. The remote avatar normally eases toward _target_pos at walk speed;
	# a big jump would otherwise slide it clear across the screen ("sideways out of
	# view"). Real walk deltas at ~15 Hz are tiny, so only a teleport trips this.
	if position.distance_to(pos) > 6.0:
		position = pos
	_target_pos = pos
	_target_yaw = yaw
	moving = is_moving and not is_sitting
	# Mirror the peer's sit/stand so a seated friend folds onto the bench instead of
	# standing through it. Edge-triggered: sit() snaps onto the seat, stand() rises.
	if is_sitting and not _sitting:
		sit(pos, yaw)
	elif not is_sitting and _sitting:
		stand()
...
func _build_primitive() -> void:
	...
	# Finish drives the whole chassis: gold (first-batch "naked robot" entry pass)
	# by default. An editor-picked painted suit (body index > 0) overrides the
	# chassis color with a BODY_COLOR + classic navy joints. `white`/`navy` keep
	# their names (chassis / details) so the rest of the builder is unchanged.
	var fin: Dictionary = FINISHES.get(str(outfit.get("finish", DEFAULT_FINISH)), FINISHES[DEFAULT_FINISH])
	var body_col: Color = fin["body"]
	var detail_col: Color = fin["detail"]
	if int(outfit.get("body", 0)) > 0:
		body_col = BODY_COLORS[int(outfit["body"]) % BODY_COLORS.size()]
		detail_col = Color(0.10, 0.12, 0.17)
	var visor_col: Color = VISOR_COLORS[int(outfit.get("visor", 0)) % VISOR_COLORS.size()]
	# ELAnaut finish: glossy chassis, deep detail joints, glowing cyan face
	var white := toon_mat(body_col, float(fin.get("rim", 0.45)), true, 0.0, 0.5, float(fin.get("spec", 0.7)))
	var navy := toon_mat(detail_col, 0.3, true, 0.0, 0.5, 0.5)
...
func _make_arm(mat: Material, hand_mat: Material, side: float) -> Node3D:
	...
	sjm.mesh = sj
	sjm.material_override = _chassis_mat if _chassis_mat else toon_mat(Color(0.95, 0.95, 0.97), 0.4, true, 0.0, 0.5, 0.7)
...
func _process(delta: float) -> void:
	_t += delta
	if _remote:
		# A seated remote stays put on the seat — don't glide it (sit() already
		# placed it); gliding would drag it off the bench toward _target_pos.
		if not _sitting:
