...
## DDRM on-device proof toggle (LOCAL key, no chain). When true, Verse load mounts a
## decrypted gold cube at y=2 IFF the encrypt->store->fetch->decrypt->render path works.
const DDRM_SELFTEST := false
...
# Ela City — the second world. It ships as its OWN pack (ela_city.pck) and is
# mounted + built the first time you travel there. Lands are isolated: no
# walking between worlds, the Worlds tab is the only door.
const ELA_CITY := Vector3(0, 0, -200)
const MALL_IN := Vector3(120, 0, -200)   # the mall's interior hall (x>30 ⇒ "inside")
...
var _world_root: Node3D            # world packs build into this
var _city_built := false
var _portals: Array = []           # {at, to, yaw} walk-through doorways (mall…)
var _talkers: Array = []           # {node, lines, i} tap-to-chat characters
var _inside_kind := ""             # "" | "house" | "mall"
...
var _door_cd := 0.0
var _riding := false   # being carried by an escalator — input + clamps paused
...
func _ready() -> void:
	...
	# DDRM on-device self-test (see DDRM_SELFTEST). Deferred so the Hey runtime +
	# the HeyVerse singleton are settled before pack/load.
	if DDRM_SELFTEST:
		get_tree().create_timer(2.0).timeout.connect(func() -> void:
			print(VerseItems.selftest(self)))
	...
	_hud.travel_pressed.connect(func() -> void:
		if player.position.z < -100.0:
			_travel_to(Vector3(0, 0, 1.5), 0.0)
		elif _ensure_city():
			_travel_to(ELA_CITY + Vector3(0, 0, 14), PI))
	...
	# saved spot in Ela City (or its mall)? mount that land before landing
	if back.z < -100.0 and not _ensure_city():
		back = Vector3(0, 0, 1.5)
	...
	# Net must keep draining app commands WHILE PAUSED: dock-exit pauses the
	# whole tree, and without this the "wake" on returning to the tab was
	# never read — the verse looked permanently stuck.
	Net.process_mode = Node.PROCESS_MODE_ALWAYS
	Net.me_updated.connect(_on_me_updated)
	# accepted a friend's invite → land in THEIR world
	Net.join_world.connect(func(zone: String) -> void:
		if zone == "city":
			if _ensure_city():
				_travel_to(ELA_CITY + Vector3(0, 0, 14), PI)
		else:
	...
	if Net.is_app():
		...
		_hud.set_desktop_mode()
	Net.notify_ready()
	if _editor:
		_editor.active(true)   # boot lands in-world, so the creator FAB is live now
	_spawn_user_items()   # show everything already placed on your land (world-space)

	# pack-only test harness: `godot --main-pack verse.pck -- --city-test`
	# exercises the exact world-pack mount path Android uses.
	if OS.get_cmdline_user_args().has("--city-test"):
		var tt := get_tree().create_timer(2.0)
		tt.timeout.connect(func() -> void:
			print("CITY-TEST: ensure=", _ensure_city())
			_teleport(ELA_CITY + Vector3(0, 0, 14), PI)
			print("CITY-TEST: at ", player.position)
			var tq := get_tree().create_timer(2.0)
			tq.timeout.connect(func() -> void: get_tree().quit()))
...
func _walk_to_screen(screen_pos: Vector2) -> void:
	...
	if player.is_sitting():
		player.stand()
	_sit_bench = {}
	# a placed catalog object / building: tap to open its info / move / remove
	# sheet — works indoors AND outdoors (not gated to the yard).
	if not _dragged:
		for n in _placed_entry.keys():
			if not is_instance_valid(n):
				continue
			var nid := str((_placed_entry[n] as Dictionary).get("catalog", ""))
			if _is_building(nid):
				# big: select if the tapped GROUND point falls within its footprint
				# (tapping a far wall is nowhere near the centre on screen)
				var bp := (n as Node3D).global_position
				if Vector2(bp.x - _target.x, bp.z - _target.z).length() < _bld_foot(nid):
					_select_placed(n)
					return
			else:
				var nsp := cam.unproject_position((n as Node3D).global_position + Vector3(0, 0.4, 0))
				if nsp.distance_to(screen_pos) < 55.0:
					_select_placed(n)
					return
	...
	if not _dragged and (not _inside or _inside_kind == "mall"):
		# the signpost: tap it to read it (outdoors only)
		if not _inside and _sign_pos != Vector3.ZERO:
			...
			if ssp.distance_to(screen_pos) < 48.0 \
				...
				_hud.show_sign()
				return
		# a character: tap them to hear what they have to say
		for tk in _talkers:
			var td: Dictionary = tk
			var tn: Node3D = td["node"]
			if not is_instance_valid(tn):
				continue
			var tsp := cam.unproject_position(tn.global_position + Vector3(0, 1.2, 0))
			if tsp.distance_to(screen_pos) < 60.0:
				_talker_say(td)
				return
...
func _process(delta: float) -> void:
	...
	if not _inside and not player.is_sitting():
		...
		# lands are isolated — each clamps to its own ground
		if player.position.z < -100.0:
			player.position.x = clampf(player.position.x, ELA_CITY.x - 28.0, ELA_CITY.x + 28.0)
			player.position.z = clampf(player.position.z, ELA_CITY.z - 28.0, ELA_CITY.z + 28.0)
		elif _started:
			player.position.x = clampf(player.position.x, -38.0, 38.0)
			player.position.z = clampf(player.position.z, -38.0, 38.0)
	...
	if _inside and _inside_kind == "mall":
		# the mall: its floors live at x offsets of 40 (hall, L2, L3, store
		# rooms) — clamp to the floor you're ON. Paused while an escalator
		# ride owns the avatar.
		if not player.is_sitting() and not _riding:
			player.position.y = 0.0
			player.position = _resolve_obstacles(player.position)
			var zi := clampi(int(roundf((player.position.x - MALL_IN.x) / 40.0)), 0, 6)
			var mz := MALL_IN + Vector3(40.0 * float(zi), 0, 0)
			var bx := 11.2
			var bz_lo := -5.9
			var bz_hi := 6.3
			if zi == 1:
				...
				bz_lo = -5.2
				bz_hi = 5.2
			player.position.x = clampf(player.position.x, mz.x - bx, mz.x + bx)
			player.position.z = clampf(player.position.z, mz.z + bz_lo, mz.z + bz_hi)
	elif _inside:
		if _loft_root:
	...
	if not HOME_ONLY and _door_cd == 0.0:
		if _inside and _inside_kind == "house":
			...
			for dpos in _doors:
				...
				if player.position.distance_to(d3) < 0.75:
					_enter_house(d3)
					break
		# generic walk-through doorways (the mall, future world doors) — plus
		# escalator RIDES ("ride" key) and elevator floor sheets ("lift" key)
		if _door_cd == 0.0:
			for p0 in _portals:
				var pd: Dictionary = p0
				var pat: Vector3 = pd["at"]
				if player.position.distance_to(pat) < 0.8:
					_door_cd = 1.2
					if pd.has("lift"):
						_open_lift(pd)
					elif pd.has("ride"):
						...
						var pto: Vector3 = pd["to"]
						var pyaw := float(pd["yaw"])
						_hud.fade(func() -> void: _teleport(pto, pyaw))
					break
	...
	if _net_accum <= 0.0 and _started:
		...
		Net.send_move(player.position, player.rotation.y, _moving, player.is_sitting())
	...
	var ps_all: Dictionary = Net.peers()
	var ps: Dictionary = {}
	var my_city := player.position.z < -100.0
	for did0 in ps_all.keys():
		var info0: Dictionary = ps_all[did0]
		var pz := str(info0.get("zone", ""))
		# friends render only in the world they're actually in
		if pz == "" or (pz == "city") == my_city:
			ps[did0] = info0
	for did in ps.keys():
		var p: Dictionary = ps[did]
		if not _remotes.has(did) or not is_instance_valid(_remotes[did]):
			var av := VerseAvatar.new()
			add_child(av)
			av.position = p.get("pos", Vector3.ZERO)
			av.setup(p.get("color", Color.WHITE), str(p.get("name", "friend")), true,
				VerseAvatar.resolve_outfit(str(did), p.get("outfit", {})))
			_remotes[did] = av
		var av2: VerseAvatar = _remotes[did]
		av2.set_remote_state(p.get("pos", av2.position), float(p.get("yaw", 0.0)), bool(p.get("moving", false)), bool(p.get("sitting", false)))
		# Remote avatars are visual GHOSTS — they must NOT run per-frame obstacle
		# resolution: in the dense city/mall that iterates the whole obstacle +
		# box set for every remote every frame (a big CPU spike + crash risk when
		# a guest walks). Their position is already interpolated client-side.
	var gone: Array = []
	...
	# camera trails the avatar (and rides up to the active floor). The look
	# target sits ABOVE the head, which drops the avatar to the lower third
	# of a phone screen — one-handed taps land on the world ahead, not on
	# your own robot.
	if not _edit_mode:
		var follow := player.position + Vector3(0, 1.4, 0)
		# while placing/moving a BUILDING the camera trails the building, so you can
		# drag it across the land and the view pans with it (not stuck on the avatar)
		var bnode: Node3D = null
		if _placing and _place_node and _place_kind == "building":
			bnode = _place_node
		elif _move_mode and _sel_node and is_instance_valid(_sel_node) and _is_building(str((_placed_entry.get(_sel_node, {}) as Dictionary).get("catalog", ""))):
			bnode = _sel_node
		if bnode:
			follow = bnode.global_position + Vector3(0, 1.4, 0)
...
## One ambient track at a time, picked by land: warm at home, calm-futuristic
## in Ela City. Crossfades on travel.
var _music_mode := ""
func _ensure_music(m: String) -> void:
	if _home_music != null and _music_mode == m:
		return
	if _home_music != null:
		_home_music.fade_out()
...
func _on_ui_cmd(cmd: String) -> void:
	if cmd.begins_with("hang:"):
		...
		return
	if cmd.begins_with("lift_pick:"):
		if not _lift_pending.is_empty():
			var pdl := _lift_pending
			_lift_pending = {}
			_lift_done(pdl, int(cmd.substr(10)))
		return
	# ── creator editor, driven by the native Library / Edit-avatar sheets ──
	if cmd.begins_with("finish:"):
		_ed_finish(cmd.substr(7))
		return
	if cmd.begins_with("wear:"):
		_ed_wear(cmd.substr(5))
		return
	if cmd.begins_with("place:"):
		_ed_pick(cmd.substr(6))
		return
	# avatar PARTS chosen as owned items (direct set, not cycle) from the native
	# Edit-avatar sheet — these become NFT parts you own/buy.
	if cmd.begins_with("eyes_set:"):
		_set_trait("eyes", int(cmd.substr(9)))
		return
	if cmd.begins_with("fins_set:"):
		_set_trait("fins", int(cmd.substr(9)))
		return
	match cmd:
		"place_rotate":
			_ed_rotate()
		...
		"move_on":
			if _sel_node:
				_move_mode = true   # "Move" chosen on the object sheet → now it follows your finger
				if _is_building(str((_placed_entry.get(_sel_node, {}) as Dictionary).get("catalog", ""))):
		...
		"save":
			_save_spawn()
		"sleep":
			# TRUE shutdown-feel: save, silence the music, pause the whole
			# game tree (the app also detaches the render surface)
			_save_spawn()
			if _home_music:
				_home_music.queue_free()
				_home_music = null
				_music_mode = ""
		...
		"wake":
			...
			if _started:
				_ensure_music("city" if player.position.z < -100.0 else "home")
		"edit_on":
			# avatar close-up: portrait camera on the robot while the app's
			# editor overlay is open (taps are ignored, changes apply live).
			# Hide the name tag so it doesn't float over his head in the close-up.
			_edit_mode = true
			_moving = false
			marker.visible = false
			...
			cam.position = Vector3(0, 1.55, 3.1)
...
## A tapped character says their next line in a floating bubble.
func _talker_say(td: Dictionary) -> void:
	var tn: Node3D = td["node"]
	var lines: Array = td["lines"]
	var i := int(td.get("i", 0))
	td["i"] = (i + 1) % lines.size()
	var text := str(lines[i])
	# "silent" talkers keep ALL their text in the app's popup module — the
	# in-world bubble only shows where there is no app (desktop preview)
	var no_bubble := bool(td.get("silent", false)) and Net.is_app()
	if not no_bubble:
		var bub: Label3D = tn.get_node_or_null("VerseBubble")
		if bub == null:
			bub = Label3D.new()
			bub.name = "VerseBubble"
			bub.position = Vector3(0, 1.76, 0)
			bub.font_size = 44
			bub.pixel_size = 0.004
			bub.billboard = BaseMaterial3D.BILLBOARD_ENABLED
			bub.modulate = Color(0.95, 0.97, 1.0)
			bub.outline_size = 10
			tn.add_child(bub)
		bub.text = text
		bub.visible = true
		...
		t.timeout.connect(func() -> void:
			if is_instance_valid(bub) and bub.text == text:
				bub.visible = false)
	# in the Hey app, characters can also open a popup sheet (Sash's FAQ)
	if td.has("sheet") and Net.is_app():
		Net.open_sheet(str(td["sheet"]))


## Fade-travel between worlds (Worlds sheet / desktop menu).
func _travel_to(pos: Vector3, yaw: float) -> void:
	_hud.fade(func() -> void: _teleport(pos, yaw))


## Worlds are separate packs. Ela City lives in its own ela_city.pck: mount
## it (APK assets, then downloaded user:// packs) and build it on first
## visit. In the editor the source script is present, so no pack is needed.
func _ensure_city() -> bool:
	if _city_built:
		return true
	if not ResourceLoader.exists("res://worlds/ela_city.gd"):
		ProjectSettings.load_resource_pack("res://worlds/ela_city.pck")
	if not ResourceLoader.exists("res://worlds/ela_city.gd"):
		ProjectSettings.load_resource_pack("user://worlds/ela_city.pck")
	if not ResourceLoader.exists("res://worlds/ela_city.gd"):
		print("hey-verse: ela_city world pack not found")
		return false
	var s: GDScript = load("res://worlds/ela_city.gd")
	if s == null or _world_root == null:
		return false
	s.new().build(_world_root, self)
	_city_built = true
	return true


var _edit_mode := false   # app's avatar editor open: close-up cam, no walking
var _bound_did := ""
...
# ─────────────────────────────────────────── creator editor (Dress / Build) ──
# In-Verse overlay (editor.gd) to dress the avatar (chassis finish + catalog
# headwear) and grid-place catalog objects in the home. Local-first: every
# catalog item is available in test; the manifest furniture entries get a
# {"catalog": id} field that _spawn_saved_furniture resolves via VerseCatalog.
const PLACE_GRID := 0.5

var _editor: CanvasLayer
var _placing := false
var _place_id := ""
var _place_kind := ""
var _place_foot := 0.45
var _place_node: Node3D
var _place_rot := 0.0
var _edit_placed := false
var _move_mode := false   # selected object only follows the finger after "Move" is chosen
var _sel_node: Node3D
var _furni_room: Node3D
var _placed_entry: Dictionary = {}   # placed Node3D -> its manifest furniture entry
var _grid_vis: MeshInstance3D
var _user_items: Node3D              # WORLD-SPACE container for placed catalog items


## Always-visible, origin-anchored container for placed catalog items. The home
## INTERIOR room is offset in world space (x>30), so parenting a world-positioned
## item to it dropped the item inside the house — invisible from the yard. This
## container sits at the scene origin, so item.position == world position and the
## object appears exactly where you stand, indoors OR in the garden.
func _user_root() -> Node3D:
	if _user_items == null or not is_instance_valid(_user_items):
		_user_items = Node3D.new()
		_user_items.name = "UserItems"
		add_child(_user_items)
	return _user_items


## In the app the native Hey dock + Library / Edit-avatar sheets drive the editor
## (via ui commands), so the in-Godot overlay is DESKTOP-ONLY.
func _setup_editor() -> void:
	if Net.is_app():
		return
	_editor = preload("res://editor.gd").new()
	add_child(_editor)
	_editor.finish_picked.connect(_ed_finish)
	_editor.hat_picked.connect(_ed_wear)
	_editor.item_picked.connect(_ed_pick)
	_editor.rotate_pressed.connect(_ed_rotate)
	_editor.delete_pressed.connect(_ed_delete)
	_editor.place_confirmed.connect(_ed_confirm)
	_editor.place_cancelled.connect(_ed_cancel)
	_editor.edit_mode_pressed.connect(_ed_begin_edit)
	_editor.active(false)


## Recolor the whole chassis (gold / silver / obsidian / classic). Finishes only
## show with no painted suit, so clear the body index + primitive hat.
func _ed_finish(id: String) -> void:
	var o: Dictionary = _manifest.get("outfit", {})
	o["finish"] = id
	o["body"] = 0
	o.erase("hat")
	_manifest["outfit"] = o
...
## Wear a catalog headwear ("" = bare). Mounts live + persists in the outfit.
func _ed_wear(id: String) -> void:
	var o: Dictionary = _manifest.get("outfit", {})
	o["catalog_hat"] = id
	o.erase("hat")
	_manifest["outfit"] = o
	_save_manifest()
	player.set_catalog_hat(id)


## Start placing a catalog object: spawn it in front of the player on the grid,
## ringed gold, and drag it with floor taps until Place.
func _ed_pick(id: String) -> void:
	_ed_cancel()
	# You own 1 of each: if it's ALREADY placed, jump to it to move/remove instead
	# of erroring — tapping "Place" on an owned-placed item opens its sheet.
	if _placed_count(id) >= 1:
		var existing := _find_placed(id)
		if existing:
			_select_placed(existing)
		return
	var n: Node3D = _build_item(id)
	if n == null:
		return
	_place_id = id
	_place_kind = _kind_of(id)
	_place_foot = _footprint_for(_place_kind)
	var is_bld := _place_kind == "building"
	if is_bld:
		_building_overview(true)   # zoom out so the whole plot is visible
	var fwd := Vector3(sin(player.rotation.y), 0.0, cos(player.rotation.y))
	_user_root().add_child(n)
	# buildings spawn well clear of the avatar (they're big) and aren't clamped to small obstacles
	var gp := _snap_kind(player.global_position + fwd * (15.0 if is_bld else 1.6), _place_kind)
	if not is_bld:
		gp = _clamp_placement(gp, _place_foot)
	n.position = gp
	_place_rot = _auto_yaw(gp)
	n.rotation.y = _place_rot
	_ring(n, true)
	_place_node = n
	_placing = true
	_show_grid(true)
	if _editor:
		_editor.show_place_actions()
	else:
		Net.open_sheet("placing:" + id)   # app shows the Rotate / Place / Cancel bar


func _ed_rotate() -> void:
	if _placing and _place_node:
		_place_rot = fmod(_place_rot + PI / 8.0, TAU)
		_place_node.rotation.y = _place_rot
	elif _sel_node:
		_sel_node.rotation.y = fmod(_sel_node.rotation.y + PI / 8.0, TAU)
		_sync_placed(_sel_node)
		var e = _placed_entry.get(_sel_node)
		if e != null:
			e["rot_y"] = _sel_node.rotation.y
			_save_manifest()


func _ed_confirm() -> void:
	if _place_node == null:
		_ed_cancel()
		return
	_ring(_place_node, false)
	if _place_kind != "building":
		_place_node.position = _clamp_placement(_place_node.position, _place_foot)
	var p := _place_node.position
	var entry := {"catalog": _place_id, "pos": [p.x, p.y, p.z], "rot_y": _place_node.rotation.y}
	var fl: Array = _manifest.get("furniture", [])
	fl.append(entry)
	_manifest["furniture"] = fl
	_save_manifest()
	_placed_entry[_place_node] = entry
	_register_placed(_place_node, _place_id)
	_place_node = null
	_placing = false
	_show_grid(false)
	_building_overview(false)
	if _editor:
		_editor.open_panel(true)   # back to the list to place more


func _ed_cancel() -> void:
	if _place_node:
		_place_node.queue_free()
		_place_node = null
	_placing = false
	if _sel_node and is_instance_valid(_sel_node):
		# finished editing a placed item — re-solidify it where it now sits
		_register_placed(_sel_node, str((_placed_entry.get(_sel_node, {}) as Dictionary).get("catalog", "")))
		_ring(_sel_node, false)
	_sel_node = null
	_edit_placed = false
	_move_mode = false
	_show_grid(false)
	_building_overview(false)
	if _editor:
		_editor.hide_actions()


## Enter "move/delete placed" mode: tap near a placed object to pick it.
func _ed_begin_edit() -> void:
	_ed_cancel()
	_edit_placed = true
	_move_mode = true
	_show_grid(true)
	if _editor:
		_editor.open_panel(false)
		_editor.show_edit_actions()


func _ed_delete() -> void:
	if _sel_node == null:
		return
	_unregister_placed(_sel_node)
	var e = _placed_entry.get(_sel_node)
	if e != null:
		var fl: Array = _manifest.get("furniture", [])
		fl.erase(e)
...
## All editor world-touches: move the active/selected object on the grid; in
## edit mode a press with nothing selected picks the nearest placed object.
func _editor_touch(event: InputEvent) -> void:
	...
	if dist <= 0.0:
		return
	var gp: Vector3 = from + dir * dist
	if _placing and _place_node:
		var np := _clamp_placement(_snap_kind(gp, _place_kind), _place_foot)
		_place_node.position = np
		if _against_wall(np):   # snap rotation flush to the wall it's pushed against
			_place_rot = _auto_yaw(np)
			_place_node.rotation.y = _place_rot
	elif _edit_placed and _move_mode and _sel_node:
		# only follows the finger once "Move" was chosen — selecting must NOT shift it
		var kind := _kind_of(str((_placed_entry.get(_sel_node, {}) as Dictionary).get("catalog", "")))
		var np2 := _snap_kind(gp, kind)
		if kind != "building":
			np2 = _clamp_placement(np2, _footprint_for(kind))
		_sel_node.position = np2
		_sync_placed(_sel_node)
		var e = _placed_entry.get(_sel_node)
		if e != null:
			e["pos"] = [np2.x, np2.y, np2.z]
	elif _edit_placed and not _move_mode and press and _sel_node == null:
		_select_nearest(gp)


func _against_wall(pos: Vector3) -> bool:
	if not (_inside and _inside_kind == "house" and _furni_room and is_instance_valid(_furni_room)):
		return false
	var c := _furni_room.global_position
	return pos.z <= c.z - 5.3 + 0.9 or pos.x <= c.x - 7.3 + 0.9 or pos.x >= c.x + 7.3 - 0.9


## Select a placed object the player tapped: ring it, pull its solid out so it can
## be moved freely, and ask the app to open its info / move / remove sheet.
func _select_placed(node: Node3D) -> void:
	if _sel_node and is_instance_valid(_sel_node):
		_register_placed(_sel_node, str((_placed_entry.get(_sel_node, {}) as Dictionary).get("catalog", "")))
		_ring(_sel_node, false)
	_sel_node = node
	_edit_placed = true
	_move_mode = false   # the sheet opens; dragging only after "Move" is chosen
	_unregister_placed(node)
	_ring(node, true)
	Net.open_sheet("item:" + str((_placed_entry.get(node, {}) as Dictionary).get("catalog", "")))


func _select_nearest(gp: Vector3) -> void:
	var best: Node3D = null
	var bestd := 1.6
	for n in _placed_entry.keys():
		if not is_instance_valid(n):
			continue
		var d := Vector2(n.position.x - gp.x, n.position.z - gp.z).length()
		if d < bestd:
			bestd = d
			best = n
	if best:
		_sel_node = best
		_move_mode = true   # desktop editor moves immediately + shows the bar
		_unregister_placed(best)
		_ring(best, true)
		if _editor:
			_editor.show_edit_actions()


## "Sit here" from a seating object's sheet: walk to its front edge + sit down.
func _sit_on_selected() -> void:
	if _sel_node == null or not is_instance_valid(_sel_node):
		_ed_cancel()
		return
	var id := str((_placed_entry.get(_sel_node, {}) as Dictionary).get("catalog", ""))
	if _kind_of(id) != "seating":
		_ed_cancel()
		return
	var ay := _sel_node.rotation.y
	var bp := Vector3(_sel_node.global_position.x, 0.0, _sel_node.global_position.z)
	_ed_cancel()   # re-registers the seat as a solid/bench + deselects
	_sit_bench = {"pos": bp, "yaw": ay}
	_target = bp + Vector3(sin(ay), 0.0, cos(ay)) * 0.85
	_moving = true
	marker.global_position = _target + Vector3(0.0, 0.03, 0.0)
	marker.visible = true
...
## A gold selection ring under the active/selected object.
func _ring(n: Node3D, on: bool) -> void:
	var r := n.get_node_or_null("SelRing")
	if on and r == null:
		var d := CylinderMesh.new()
		d.top_radius = 0.62
		d.bottom_radius = 0.62
		d.height = 0.02
		d.radial_segments = 22
		var mi := MeshInstance3D.new()
		mi.name = "SelRing"
		mi.mesh = d
		mi.material_override = VerseAvatar.glow_mat(Color(0.83, 0.72, 0.29), 1.3)
		mi.position.y = 0.025
...
func _snap(p: Vector3) -> Vector3:
	return Vector3(round(p.x / PLACE_GRID) * PLACE_GRID, 0.0, round(p.z / PLACE_GRID) * PLACE_GRID)


## Pull the camera WAY back + steeper while placing/moving a building, so the whole
## plot is visible and a big house isn't sitting on top of the avatar.
func _building_overview(on: bool) -> void:
	if _edit_mode:
		return
	if on:
		cam.position = Vector3(0.0, 28.0, 24.0)
		cam.rotation_degrees = Vector3(-54.0, 0.0, 0.0)
	else:
		cam.position = Vector3(0.0, 8.5, 10.5)
		cam.rotation_degrees = Vector3(-37.0, 0.0, 0.0)


# ── placement quality: scarcity, footprint, obstacle/wall clamp, auto-orient ──
const _FOOT := {"seating": 0.55, "table": 0.7, "lighting": 0.35, "wallart": 0.25, "plant": 0.45, "decor": 0.45}
const WALL_Y := 1.5   # wall art hangs at this height (not flat on the floor)

## Direct trait set (eyes/antenna) chosen as an owned NFT part from the app.
func _set_trait(key: String, n: int) -> void:
	var saved: Dictionary = _manifest.get("outfit", {})
	saved[key] = n
	_manifest["outfit"] = saved
	_save_manifest()
...
## id -> full catalog record (name/kind/rarity/description); built once.
var _cat_meta: Dictionary = {}
func _catalog_rec(id: String) -> Dictionary:
	if _cat_meta.is_empty():
		for it in VerseCatalog.all():
			_cat_meta[str((it as Dictionary).get("id", ""))] = it
	return _cat_meta.get(id, {})


## Buildings are a second placeable registry (VerseBuildings). They're BIG, you
## walk inside (no collision blob), and they place in the open yard (no clamp).
var _bld_ids: Dictionary = {}   # building id -> select/footprint radius (metres)
func _is_building(id: String) -> bool:
	if _bld_ids.is_empty():
		for it in VerseBuildings.all():
			var d: Dictionary = it
			var fp: Array = d.get("footprint", [12.0, 12.0])
			_bld_ids[str(d.get("id", ""))] = maxf(float(fp[0]), float(fp[1])) * 0.5 + 1.5
	return _bld_ids.has(id)


## Generous pick radius for a building (tap anywhere on its footprint to select it).
func _bld_foot(id: String) -> float:
	return float(_bld_ids.get(id, 10.0))


## Build a placeable by id from whichever registry owns it.
func _build_item(id: String) -> Node3D:
	if _is_building(id):
		return VerseBuildings.build(id)
	return VerseCatalog.build(id)


func _kind_of(id: String) -> String:
	if _is_building(id):
		return "building"
	return str(_catalog_rec(id).get("kind", ""))


func _footprint_for(kind: String) -> float:
	return float(_FOOT.get(kind, 0.45))


## Local-first NFT scarcity: you own 1 of each. How many of `id` are already placed.
func _placed_count(id: String) -> int:
	var c := 0
	for e in _manifest.get("furniture", []):
		if str((e as Dictionary).get("catalog", "")) == id:
			c += 1
	return c


## Snap to grid; wall art rides at wall height, everything else on the floor.
func _snap_kind(p: Vector3, kind: String) -> Vector3:
	var y := WALL_Y if kind == "wallart" else 0.0
	return Vector3(round(p.x / PLACE_GRID) * PLACE_GRID, y, round(p.z / PLACE_GRID) * PLACE_GRID)


## Keep a placement legal: push it out of houses / trees / benches / pond / other
## placed items, and clamp inside the home walls so nothing lands half-in-a-wall.
func _clamp_placement(pos: Vector3, foot: float) -> Vector3:
	var y := pos.y
	pos = _resolve_obstacles(pos, foot)
	if _inside and _inside_kind == "house" and _furni_room and is_instance_valid(_furni_room):
		var c := _furni_room.global_position
		pos.x = clampf(pos.x, c.x - 7.3 + foot, c.x + 7.3 - foot)
		pos.z = clampf(pos.z, c.z - 5.3 + foot, c.z + 5.3 - foot)
	pos.y = y
	return pos


## Auto-orient: against a home wall, back to it (face into the room); else face
## the player who placed it.
func _auto_yaw(pos: Vector3) -> float:
	if _inside and _inside_kind == "house" and _furni_room and is_instance_valid(_furni_room):
		var c := _furni_room.global_position
		if pos.z <= c.z - 5.3 + 0.9:
			return 0.0
		if pos.x <= c.x - 7.3 + 0.9:
			return PI / 2.0
		if pos.x >= c.x + 7.3 - 0.9:
			return -PI / 2.0
	return atan2(player.global_position.x - pos.x, player.global_position.z - pos.z)


## A placed catalog object becomes a SOLID (walk around it / no stacking) and, if
## it's seating, a sittable bench. Stored as node meta so move/delete can update.
func _register_placed(node: Node3D, id: String) -> void:
	var kind := _kind_of(id)
	if kind == "building":
		return   # you WALK INTO buildings — no collision blob, no seat
	if kind == "wallart":
		return   # on the wall — doesn't block the floor or seat anyone
	var wp := node.global_position
	var solid := {"pos": Vector3(wp.x, 0.0, wp.z), "r": _footprint_for(kind)}
	_obstacles.append(solid)
	node.set_meta("solid", solid)
	if kind == "seating":
		var seat := {"pos": Vector3(wp.x, 0.0, wp.z), "yaw": node.rotation.y}
		_benches.append(seat)
		node.set_meta("seat", seat)


func _unregister_placed(node: Node3D) -> void:
	if node.has_meta("solid"):
		_obstacles.erase(node.get_meta("solid"))
		node.remove_meta("solid")
	if node.has_meta("seat"):
		_benches.erase(node.get_meta("seat"))
		node.remove_meta("seat")


## Keep a moved object's solid/seat in sync with its new spot.
func _sync_placed(node: Node3D) -> void:
	var wp := node.global_position
	if node.has_meta("solid"):
		(node.get_meta("solid") as Dictionary)["pos"] = Vector3(wp.x, 0.0, wp.z)
	if node.has_meta("seat"):
		var s: Dictionary = node.get_meta("seat")
		s["pos"] = Vector3(wp.x, 0.0, wp.z)
		s["yaw"] = node.rotation.y


func _show_grid(on: bool) -> void:
	if on and (_grid_vis == null or not is_instance_valid(_grid_vis)):
		_grid_vis = _make_grid()
	if _grid_vis == null:
		return
	if on:
		var room := _user_root()
		if _grid_vis.get_parent() != room:
			if _grid_vis.get_parent():
				_grid_vis.get_parent().remove_child(_grid_vis)
			room.add_child(_grid_vis)
		_grid_vis.position = Vector3(_snap(player.global_position).x, 0.02, _snap(player.global_position).z)
	_grid_vis.visible = on


func _make_grid() -> MeshInstance3D:
	var pm := PlaneMesh.new()
	pm.size = Vector2(16, 16)   # 16 m / 32 cells = 0.5 m grid == PLACE_GRID
	var mi := MeshInstance3D.new()
	mi.mesh = pm
	var sh := Shader.new()
	sh.code = "shader_type spatial;\nrender_mode unshaded, cull_disabled, depth_draw_opaque;\nuniform vec4 line_col : source_color = vec4(0.83, 0.72, 0.29, 0.5);\nuniform float cells = 32.0;\nvoid fragment() {\n\tvec2 g = abs(fract(UV * cells - 0.5) - 0.5) / fwidth(UV * cells);\n\tfloat line = 1.0 - min(min(g.x, g.y), 1.0);\n\tALBEDO = line_col.rgb;\n\tALPHA = line * line_col.a;\n}\n"
...
func _spawn_saved_furniture(room: Node3D) -> void:
	...
	for entry in _manifest.get("furniture", []):
		...
		if e.has("catalog"):
			continue   # catalog items are WORLD-space, spawned once at boot (_spawn_user_items)
...
## Spawn placed CATALOG items at BOOT into the world-space container — independent
## of the home build, so they're visible whether you're in the yard or indoors
## (the home build only runs when you enter the house). Idempotent: clears first.
func _spawn_user_items() -> void:
	for n in _placed_entry.keys():
	...
	for entry in _manifest.get("furniture", []):
		var e: Dictionary = entry
		if not e.has("catalog"):
			continue
		var node: Node3D = _build_item(str(e["catalog"]))
		if node == null:
			continue
		var p: Array = e.get("pos", [0.0, 0.0, 0.0])
		_user_root().add_child(node)
		node.position = Vector3(float(p[0]), float(p[1]), float(p[2]))
		node.rotation.y = float(e.get("rot_y", 0.0))
		_placed_entry[node] = e
		_register_placed(node, str(e["catalog"]))


## The placed instance of a catalog id (you own 1, so at most one), or null.
func _find_placed(id: String) -> Node3D:
	for n in _placed_entry.keys():
		if is_instance_valid(n) and str((_placed_entry[n] as Dictionary).get("catalog", "")) == id:
			return n
	return null
...
func _apply_preset(i: int) -> void:
	...
	# the mall keeps its OWN fixed house light — presets only restyle outdoors
	if _inside_kind == "mall":
		_apply_mall_light()
...
func _build_world() -> void:
	...
	_add_fireflies(w)
	_build_interior(w)
	_world_root = w   # world packs (Ela City…) build into this on demand
...
func _teleport(pos: Vector3, yaw: float) -> void:
	_inside = pos.x > 30.0
	_inside_kind = ("mall" if pos.z < -100.0 else "house") if _inside else ""
	...
	if _inside:
		...
		_apply_preset(_preset_idx)   # back outdoors: the day/night look returns
	player.position = pos
	...
	if _started:
		_save_spawn()
		_ensure_music("city" if pos.z < -100.0 else "home")
...
## The mall's own house light — ONE warm fixed look, immune to the outdoor
## day/night presets. Applied on entry and re-asserted over preset switches.
func _apply_mall_light() -> void:
	sun.light_color = Color(1.0, 0.94, 0.84)
	sun.light_energy = 0.3
	sun.rotation_degrees = Vector3(-58, -28, 0)
	env.ambient_light_color = Color(0.86, 0.83, 0.78)
	env.ambient_light_energy = 0.36
	env.glow_intensity = 0.5
	env.tonemap_exposure = 0.92


## An escalator portal: the avatar is CARRIED along the ramp (the camera rides
## along), then a quick fade lands it on the destination floor.
func _ride_portal(pd: Dictionary) -> void:
	_riding = true
	_stop_walk()
	_door_cd = 99.0
	var ride_to: Vector3 = pd["ride"]
	var pto: Vector3 = pd["to"]
	var pyaw := float(pd["yaw"])
	var d := ride_to - player.position
	player.rotation.y = atan2(d.x, d.z)
	player.moving = false
	var tw := create_tween()
	tw.tween_property(player, "position", ride_to, 2.3) \
...
## The elevator. In the app the floor picker is a NATIVE Hey sheet (the same
## popup module as Sash's chat): labels ride open_sheet, the pick returns as a
## "lift_pick:N" ui command. On desktop the in-game sheet stands in.
var _lift_pending: Dictionary = {}
func _open_lift(pd: Dictionary) -> void:
	_stop_walk()
	_door_cd = 99.0
	var opts: Array = pd["lift"]
	var labels: Array = []
	for o in opts:
		var od: Dictionary = o
		labels.append(str(od["label"]))
	if Net.is_app():
		_lift_pending = pd
		Net.open_sheet("lift:" + "|".join(PackedStringArray(labels)))
		return
	_hud.lift_menu(labels, func(i: int) -> void: _lift_done(pd, i))


## A floor was picked (or -1 = stay): ride, or step back so the doors close.
func _lift_done(pd: Dictionary, i: int) -> void:
	_door_cd = 1.6
	var opts: Array = pd["lift"]
	if i < 0 or i >= opts.size():
		var pat: Vector3 = pd["at"]
		var away := player.position - pat
		away.y = 0.0
		if away.length() < 0.05:
			away = Vector3(1, 0, 0)
		_target = player.position + away.normalized() * 1.3
		_moving = true
		marker.visible = false
		return
	var od2: Dictionary = opts[i]
	var pto: Vector3 = od2["to"]
	var pyaw := float(od2["yaw"])
	_hud.fade(func() -> void: _teleport(pto, pyaw))
