...
func _ready() -> void:
	...
	# DESKTOP-ONLY: starts hidden; set_desktop_mode() reveals it. On Android
	# the Hey app's own dock + popup modules drive the game.
	dock_row.visible = false
	_dock_row = dock_row
	add_child(dock_row)
...
func set_app_mode() -> void:
	_menu_btn.visible = false
	if _dock_row:
		_dock_row.visible = false
	close_menu()


## Desktop (no Hey app around the game): show the in-game dock.
func set_desktop_mode() -> void:
	if _dock_row:
		_dock_row.visible = true
...
# ── the elevator floor sheet ──────────────────────────────────────────────────
var _lift: PanelContainer
var _lift_dim: ColorRect


## Elevator UI: a centered frosted Hey sheet with one button per floor.
## `on_pick` receives the option index, or -1 for "stay here".
func lift_menu(options: Array, on_pick: Callable) -> void:
	if _lift:
		return
	_lift_dim = ColorRect.new()
	_lift_dim.color = Color(0, 0, 0, 0.45)
	_lift_dim.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	_lift_dim.mouse_filter = Control.MOUSE_FILTER_STOP
	add_child(_lift_dim)
	_lift = PanelContainer.new()
	_lift.add_theme_stylebox_override("panel", _box(Color(SHEET.r, SHEET.g, SHEET.b, 0.97)))
	_lift.custom_minimum_size = Vector2(252, 0)
	add_child(_lift)
	...
	for i in options.size():
		...
		b.custom_minimum_size = Vector2(228, 48)
		_btn_gold(b)
		var idx := i
		b.pressed.connect(func() -> void:
			_close_lift()
	...
	c.text = "stay here"
	c.flat = true
	c.custom_minimum_size = Vector2(228, 40)
	c.add_theme_color_override("font_color", MUTED)
	c.pressed.connect(func() -> void:
		_close_lift()
		on_pick.call(-1))
	v.add_child(c)
	# center it once its size resolves
	_lift.set_anchors_preset(Control.PRESET_CENTER)
	_lift.reset_size()


func _close_lift() -> void:
	if _lift:
		_lift.queue_free()
		_lift = null
	if _lift_dim:
		_lift_dim.queue_free()
		_lift_dim = null
