// AddFriendModal — paste a DID, follow that peer. Stays simpler than
// the React reference's AddFriendModal because we don't yet have the
// QR-card import flow ported (that uses the lib/profile bundle).
                ...
                <div class="fixed inset-0 z-40 flex items-start justify-center bg-black/40 p-4 pt-20" on:click=move |_: MouseEvent| open.set(false)>
                    ...
                    >
                        <header class="flex items-center justify-between">
                            <h3 class="logo-handwritten text-2xl text-primary">"Add a friend"</h3>
                            <button
                                type="button"
                        ...
                        <p class="text-xs text-muted">"Paste their did:key from their share card or QR code."</p>
                        <input
                            type="text"
                        ...
                        <button
                            ...
                            class="unfrost w-full rounded-full bg-accent hover:bg-amber-300 disabled:opacity-50 disabled:cursor-not-allowed text-accent-text font-semibold px-4 py-2 text-sm"
                        >
