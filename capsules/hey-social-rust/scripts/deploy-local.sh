#!/usr/bin/env bash
#
# Deploy hey-social-rust into the locally-installed Elastos Runtime
...
#
#   sudo bash capsules/hey-social-rust/scripts/deploy-local.sh
#
# What it does:
...
echo "=== Deploying to $DATA_DIR/hey-social-rust ==="
rm -rf "$DATA_DIR/hey-social-rust"
cp -r "$CAPSULE_DIR" "$DATA_DIR/hey-social-rust"
# Strip dev cruft that doesn't belong in the deployed capsule.
rm -rf "$DATA_DIR/hey-social-rust/scripts" \
       "$DATA_DIR/hey-social-rust/target" \
       "$DATA_DIR/hey-social-rust/Cargo.lock" \
       "$DATA_DIR/hey-social-rust/.cargo"
chown -R "$APP_USER:$APP_USER" "$DATA_DIR/hey-social-rust"
