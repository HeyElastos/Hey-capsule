#!/usr/bin/env bash
#
# Deploy hey-social-rust from THIS dev machine to a remote YunoHost
...
#
#   ./capsules/hey-social-rust/scripts/deploy-remote.sh
#   REMOTE=pc2@jothgard ./capsules/hey-social-rust/scripts/deploy-remote.sh
#
...
echo "=== [3/4] Installing into $REMOTE:$REMOTE_DATA_DIR/hey-social-rust ==="
...
rm -rf "$REMOTE_DATA_DIR/hey-social-rust"
cp -r "$REMOTE_STAGING" "$REMOTE_DATA_DIR/hey-social-rust"
chown -R "$APP_USER:$APP_USER" "$REMOTE_DATA_DIR/hey-social-rust"
...
echo "  $DOMAIN_PATH"
echo
echo "If sign-in fails, check the browser console for [hey-social-rust]"
