...
// ── Conversation ─────────────────────────────────────────────────────────
/// HEY: global connection-mode badge. Polls the carrier's `peer_paths` op (iroh 1.0
/// endpoint.remote_info, carrier PATCH 0019) and shows whether your live P2P links are
/// DIRECT (relay-free peer-to-peer) or via a RELAY (forwarded when NAT blocks a direct
/// path). End-to-end post-quantum encrypted either way — the relay only sees ciphertext.
#[component]
fn ConnBadge() -> impl IntoView {
