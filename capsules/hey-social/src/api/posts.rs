...
// Upload size ceiling. Images are compressed to WebP (≤2048px, a few hundred KB)
// BEFORE this is enforced on the final upload bytes, so this really only bounds
// raw video. 25 MB suits the on-device mobile runtime (64 MB axum body limit);
// the old 2 MB value rejected ordinary phone photos at PICK time, before
// compression ever ran. Compressed images stay well under any runtime's limit.
pub const POST_MEDIA_MAX_BYTES: usize = 25 * 1024 * 1024;
...
pub async fn ipfs_upload_media(
    ...
    web_sys::console::log_1(
        &format!("[hey-social upload] ipfs_upload_media {filename} raw={} bytes", bytes.len()).into(),
    );
    ...
    web_sys::console::log_1(
        &format!(
            "[hey-social upload] publishing {} bytes ({upload_mime})",
            upload_bytes.len()
        )
        .into(),
    );
    ...
    web_sys::console::log_1(&format!("[hey-social upload] published cid={cid}").into());
