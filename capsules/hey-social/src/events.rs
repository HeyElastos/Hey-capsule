...
pub async fn create_signed_event(
    ...
    // Provider-backed session (empty seed): the runtime holds the key and
    // signs — no passkey tap, no seed in the browser. sender_did comes from
    // the adopted session (we can't derive it without the seed).
