...
pub fn Posts() -> impl IntoView {
    ...
    let ingest_files = move |files: FileList| {
        web_sys::console::log_1(
            &format!("[hey-social upload] ingest: {} file(s)", files.length()).into(),
        );
        if files.length() == 0 {
            return;
        }
        ...
        for i in 0..files.length() {
            ...
            let mime = file.type_();
            web_sys::console::log_1(
                &format!("[hey-social upload] picked: {name} type='{mime}'").into(),
            );
            ...
            spawn_local(async move {
                let buf_value = match JsFuture::from(file.array_buffer()).await {
                    ...
                    Err(e) => {
                        web_sys::console::log_1(
                            &format!("[hey-social upload] array_buffer FAILED for {name}: {e:?}")
                                .into(),
                        );
                        error.set(format!("Could not read {name} (file not readable)"));
                ...
                web_sys::console::log_1(
                    &format!("[hey-social upload] read {} bytes for {name}", bytes.len()).into(),
                );
                ...
                // Images are compressed (WebP ≤2048px) at upload, so don't reject
                // them on RAW size here — a normal phone photo is several MB raw
                // but a few hundred KB after compression. Only gate non-images
                // (video, which passes through raw) at pick time; oversize
                // compressed images are still caught in ipfs_upload_media.
                let is_image = mime.starts_with("image/") && mime != "image/gif";
                if !is_image && bytes.len() > POST_MEDIA_MAX_BYTES {
                    error.set(format!(
                        "{name} is too large ({}) — keep it under 25 MB for now (video transcoding not yet available).",
                ...
                web_sys::console::log_1(
                    &format!("[hey-social upload] staged {name}").into(),
                );
                staged.update(|v| {
                    v.push(StagedFile {
