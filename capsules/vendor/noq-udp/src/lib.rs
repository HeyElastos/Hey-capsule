...
/// HYPER: process-wide link generation, for dead-address-family suppression.
///
/// [`UdpSocketState::send`] stops attempting an address family once sends to it have
/// repeatedly failed with `ENETUNREACH`. That verdict comes from the host **routing
/// table**, which is per-host rather than per-socket — so when the link changes, every
/// socket's conclusion about every family becomes stale at the same instant, and a single
/// process-wide counter is exactly the right scope to invalidate them all.
///
/// Defined here rather than in `imp` so the re-arm entry point exists on every platform,
/// including the ones that have no suppression to re-arm.
static HYPER_LINK_GENERATION: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);

/// HYPER: forget every "this address family has no route" conclusion, right now.
///
/// Call this on a link/interface change. Without it, a host that moves from a v4-only
/// network onto one with working IPv6 would keep suppressing IPv6 until the backoff window
/// happened to expire — up to five minutes of a family that works being skipped, which is a
/// connectivity bug rather than a saving. The suppression must follow observed reality, and
/// a link change is the single loudest signal that observed reality just changed.
///
/// Cheap enough to call unconditionally: it is one relaxed atomic increment, and the sockets
/// notice on their next send rather than being woken.
pub fn hyper_rearm_dead_families() {
    HYPER_LINK_GENERATION.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
}

/// The current link generation, read by the per-socket suppression state.
#[cfg(not(wasm_browser))]
pub(crate) fn hyper_link_generation() -> u64 {
    HYPER_LINK_GENERATION.load(std::sync::atomic::Ordering::Relaxed)
}
