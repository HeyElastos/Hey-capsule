...
/// HYPER: how long to skip sends to an address family that returned `ENETUNREACH` (the whole family
/// has no route). See [`UdpSocketState::send`]. 5 min balances killing the ~60s prober hammer against
/// how quickly a network that GAINS the family is picked back up (one re-probe per window).
const HYPER_DEAD_FAMILY_BACKOFF: Duration = Duration::from_secs(300);

/// HYPER: consecutive `ENETUNREACH` sends before an address family is treated as dead.
///
/// Suppressing on a *single* failure is too eager: a family can fail transiently for one
/// datagram while an interface is being reconfigured, and paying for that with a five-minute
/// blackout of a family that actually works is a far worse trade than a handful of extra
/// failed syscalls. Three strikes is reached in about a second at the observed probe rate,
/// so it costs essentially nothing to demand real evidence. Any success resets the count.
const HYPER_DEAD_FAMILY_STRIKES: u32 = 3;

/// HYPER: per-address-family record of "sends to this family have no route".
///
/// The fast path is deliberately lock-free. `strikes` is checked with one relaxed load on
/// every send, and the mutex is only taken once a family is actually in trouble — a working
/// network never touches it.
#[derive(Debug, Default)]
struct DeadFamily {
    /// Consecutive `ENETUNREACH` failures. Reset by any success, and by a link change.
    strikes: AtomicU32,
    /// The link generation the current verdict was reached under.
    ///
    /// When [`crate::hyper_link_generation`] moves past this, everything we concluded about
    /// this family was concluded about a network that no longer exists, and is discarded.
    generation: AtomicU64,
    /// When suppression began, once [`HYPER_DEAD_FAMILY_STRIKES`] was reached.
    since: Mutex<Option<Instant>>,
}

impl DeadFamily {
    /// Should this send be skipped entirely?
    ///
    /// Also handles re-arming: both the link-generation check and the backoff expiry clear
    /// the verdict and let the caller's datagram through as a live re-probe.
    fn should_skip(&self) -> bool {
        // A link change retires every conclusion, immediately and unconditionally.
        let generation = crate::hyper_link_generation();
        if self.generation.swap(generation, Ordering::Relaxed) != generation {
            self.strikes.store(0, Ordering::Relaxed);
            *self.since.lock().expect("poisoned lock") = None;
            return false;
        }
        // Fast path: nothing wrong with this family, no lock taken.
        if self.strikes.load(Ordering::Relaxed) < HYPER_DEAD_FAMILY_STRIKES {
            return false;
        }
        let mut since = self.since.lock().expect("poisoned lock");
        match *since {
            Some(t) if t.elapsed() < HYPER_DEAD_FAMILY_BACKOFF => true,
            _ => {
                // Window elapsed (or we struck out but never armed): let this send re-probe.
                *since = None;
                self.strikes.store(0, Ordering::Relaxed);
                false
            }
    ...
    /// A datagram got out: whatever we thought was wrong with this family, is not.
    fn note_success(&self) {
        if self.strikes.load(Ordering::Relaxed) != 0 {
            self.strikes.store(0, Ordering::Relaxed);
            *self.since.lock().expect("poisoned lock") = None;
        }
    }

    /// A send failed with `ENETUNREACH` — the whole family has no route.
    fn note_unreachable(&self) {
        let strikes = self.strikes.fetch_add(1, Ordering::Relaxed) + 1;
        if strikes >= HYPER_DEAD_FAMILY_STRIKES {
            let mut since = self.since.lock().expect("poisoned lock");
            if since.is_none() {
...
pub struct UdpSocketState {
    ...
    /// HYPER dead-family suppression, keyed by destination family so a dead IPv6 route never
    /// suppresses IPv4 (or vice versa). See [`DeadFamily`] and [`UdpSocketState::send`].
    hyper_unreach_v4: DeadFamily,
    hyper_unreach_v6: DeadFamily,
...
impl UdpSocketState {
    pub fn new(sock: UdpSockRef<'_>) -> io::Result<Self> {
        ...
        Ok(Self {
            ...
            hyper_unreach_v4: DeadFamily::default(),
            hyper_unreach_v6: DeadFamily::default(),
    ...
    pub fn send(&self, socket: UdpSockRef<'_>, transmit: &Transmit<'_>) -> io::Result<()> {
        // HYPER dead-family suppression. iroh's path prober re-sends a DISCO probe to every candidate;
        // when a candidate is in an address family this host has NO route to (e.g. a v4-only Wi-Fi and
        // a peer's cellular IPv6), each probe fails with ENETUNREACH — which this function swallows to
        // Ok and only logs. That hammers the OS + spams the log forever. We skip the syscall once a
        // family has failed repeatedly; the datagram was going to be dropped to Ok anyway, so this is
        // behaviour-preserving. `.to_canonical()` distinguishes a REAL IPv6 destination from an
        // IPv4-mapped one, so v6 failures never gate v4.
        //
        // NOTE ON WHAT THIS DOES AND DOES NOT SAVE: these datagrams die inside the host, at sendmsg,
        // because there is no route. They never reach the modem. So this saves CPU, syscalls and log
        // spam — it does NOT save radio time or cellular data, and must not be reported as though it
        // did.
        //
        // Re-arming follows observed reality rather than any hardcoded assumption about which
        // networks have which families: any success clears it, the backoff window expiring lets a
        // probe through, and a link change discards the verdict outright
        // (`crate::hyper_rearm_dead_families`).
        let family = if transmit.destination.ip().to_canonical().is_ipv6() {
            &self.hyper_unreach_v6
        } else {
            &self.hyper_unreach_v4
        };
        if family.should_skip() {
            return Ok(());
        }
        match send(self, socket.0, transmit) {
            ...
            Err(e) => {
                // Whole-family routing failure → suppress this family for a backoff window. Host-level
                // failures (EHOSTUNREACH etc.) are per-peer, not per-family, so they are NOT suppressed.
                if e.raw_os_error() == Some(libc::ENETUNREACH) {
                    family.note_unreachable();
                }
...
/// HYPER PATCH: tests for the dead-address-family suppression state machine.
///
/// Every test syncs the link generation first (one `should_skip` call), because
/// [`crate::hyper_rearm_dead_families`] is process-wide and the test binary is
/// multi-threaded — without the sync a concurrent test's re-arm would look like a link
/// change and reset the state under us.
#[cfg(test)]
mod hyper_dead_family_tests {
    use super::*;

    fn synced() -> DeadFamily {
        let f = DeadFamily::default();
        f.should_skip(); // adopt the current generation
        f
    }

    #[test]
    fn one_failure_does_not_suppress() {
        // A transient ENETUNREACH while an interface is being reconfigured must not cost a
        // five-minute blackout of a family that works.
        let f = synced();
        f.note_unreachable();
        assert!(!f.should_skip(), "one strike must not suppress");
    }

    #[test]
    fn repeated_failures_suppress() {
        let f = synced();
        for _ in 0..HYPER_DEAD_FAMILY_STRIKES {
            f.note_unreachable();
        }
        assert!(f.should_skip(), "the family has no route; stop attempting it");
    }

    #[test]
    fn success_clears_the_strikes() {
        let f = synced();
        for _ in 0..HYPER_DEAD_FAMILY_STRIKES - 1 {
            f.note_unreachable();
        }
        f.note_success();
        f.note_unreachable();
        assert!(!f.should_skip(), "a success must reset the run of failures");
    }

    #[test]
    fn success_lifts_an_active_suppression() {
        let f = synced();
        for _ in 0..HYPER_DEAD_FAMILY_STRIKES {
            f.note_unreachable();
        }
        ...
        f.note_success();
        assert!(!f.should_skip(), "a datagram got out; the family is alive");
    }

    #[test]
    fn a_link_change_re_arms_the_family() {
        // THE bug this fixes: without it, moving onto a network that has working IPv6 would
        // keep IPv6 suppressed until the backoff window happened to lapse.
        let f = synced();
        for _ in 0..HYPER_DEAD_FAMILY_STRIKES {
            f.note_unreachable();
        }
        ...
        assert!(
            !f.should_skip(),
            "a link change must discard the verdict immediately"
        );
    }

    #[test]
    fn families_are_independent() {
        // A dead IPv6 route must never suppress IPv4, or vice versa.
        let state = (synced(), synced());
        for _ in 0..HYPER_DEAD_FAMILY_STRIKES {
            state.1.note_unreachable();
        }
        assert!(!state.0.should_skip(), "v4 must be unaffected by v6");
