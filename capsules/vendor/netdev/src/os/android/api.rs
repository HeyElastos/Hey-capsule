...
fn try_init_android_context() -> bool {
    // HYPER PATCH (panic=abort safety): upstream probes ndk_context via `panic::catch_unwind`, which does
    // NOT catch under `panic = "abort"` — Hyper's required profile (FGS crash-safety). Under abort the
    // probe panic (ndk_context::android_context() when uninitialized) ABORTS the whole process at carrier
    // startup: iroh 1.0.2's `EndpointInner::bind` enumerates interfaces + their stats, hitting this path.
    // Hyper never calls `ndk_context::initialize_android_context`, so the context is definitionally
    // unavailable — report that directly, WITHOUT a catch_unwind that would abort. Per-interface byte
    // counters (rx/tx) are unused by iroh's bind and by Hyper's connection UI, so returning None for
    // Android stats is fully correct. Removable once ndk_context is initialized app-side.
    let _ = silent_panic_hook; // keep the helper + its import referenced (upstream-parity, no behavior)
    false
}
