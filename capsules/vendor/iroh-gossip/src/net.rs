...
impl Actor {
            ...
            (peer_id, res) = self.dialer.next_conn() => {
                ...
                match res {
                    ...
                    Some(Err(err)) => {
                        ...
                        if !is_active {
                            // Hey patch (per-peer dial backoff): record the failure so the next re-dial to
                            // this unreachable peer is exponentially delayed (2s→…→300s) instead of hammering
                            // every ~2s. Only for a NON-active peer, so a live neighbor is never throttled.
                            // This call site was dropped in the 0.100→0.101 re-vendor, leaving the patch
                            // inert (note_dial_failed defined but never invoked); restored here.
                            self.dialer.note_dial_failed(peer_id);
    ...
    fn handle_connection(&mut self, peer_id: EndpointId, origin: ConnOrigin, conn: Connection) {
        // Hey patch: a connection to this peer is now established (whether WE dialed it or it dialed US),
        // so clear any stale failure backoff — if it later drops, our re-dial must not be throttled.
        // NOTE: an earlier attempt to clear this only on a ≥30s "stable" close instead REGRESSED
        // connectivity — a relay-only peer that flaps <30s then got backed off to 300s and dropped to
        // 0 peers (no delivery). Clearing on establishment keeps a flapping peer PRESENT so the plumtree
        // still has a chance to form; the CPU cost of the reconnect churn is bounded by the relay pacer.
        self.dialer.note_dial_ok(peer_id);
...
struct Dialer {
    ...
    // Hey patch (per-peer dial backoff): an OFFLINE gossip member keeps being re-advertised via
    // shuffle/forward-join, so HyParView re-emits SendMessage and this dialer re-dials it every few
    // seconds — each connect() failing → sustained `dial failed` churn + background CPU. Map a peer to
    // (next_allowed_dial, next_delay). A FIRST dial and any SUCCESSFUL dial are never delayed (entry
    // absent/cleared), and a returning peer dials US, so mesh formation is unaffected; only the
    // wasteful hammering of an unreachable peer is throttled (2s → … → 300s).
    dial_backoff: HashMap<EndpointId, (n0_future::time::Instant, std::time::Duration)>,
}
...
impl Dialer {
    ...
    /// Hey patch: clear a peer's failure backoff after a SUCCESSFUL dial/connection.
    fn note_dial_ok(&mut self, endpoint_id: EndpointId) {
        self.dial_backoff.remove(&endpoint_id);
    }

    /// Hey patch: after a FAILED dial, exponentially grow the wait before the next dial (2s→…→300s).
    fn note_dial_failed(&mut self, endpoint_id: EndpointId) {
        ...
        let cur = self
            ...
            .map(|(_, d)| *d)
            .unwrap_or(BASE);
        let next_delay = (cur * 2).min(MAX);
        self.dial_backoff
            .insert(endpoint_id, (n0_future::time::Instant::now() + cur, next_delay));
    }
    ...
    fn queue_dial(&mut self, endpoint_id: EndpointId, alpn: Bytes) {
        ...
        // Hey patch: honor the per-peer failure backoff — skip re-dialing a peer that just failed until
        // its window elapses. Cleared on success (note_dial_ok), so a reachable peer is never held back.
        if let Some((next, _)) = self.dial_backoff.get(&endpoint_id) {
            if n0_future::time::Instant::now() < *next {
                return;
            }
        }
