...
impl IpSender {
    ...
    pub(super) fn poll_send(
        ...
        // Instrumentation: the byte counters below cannot be compared against an OS packet count,
        // so also count the datagrams this send puts on the wire (GSO makes that != 1).
        let total_datagrams = transmit.datagram_count() as u64;
            ...
            Poll::Ready(Ok(res)) => {
                match dst {
                    SocketAddr::V4(_) => {
                        ...
                        self.metrics.send_ipv4_datagrams.inc_by(total_datagrams);
                    }
                    SocketAddr::V6(_) => {
                        self.metrics.send_ipv6.inc_by(total_bytes);
                        self.metrics.send_ipv6_datagrams.inc_by(total_datagrams);
                    }
                }
                self.metrics.send_ip_calls.inc();
                Poll::Ready(Ok(res))
            }
            Poll::Ready(Err(err)) => Poll::Ready(Err(err)),
            Poll::Pending => Poll::Pending,
