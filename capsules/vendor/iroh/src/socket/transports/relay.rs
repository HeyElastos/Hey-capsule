...
impl RelayTransport {
    ...
    pub(crate) fn create_sender(&self) -> RelaySender {
        RelaySender {
            ...
            tokens: 256.0, // start full — first burst is never delayed
            last_refill: None,
            pace_timer: None,
        }
    }
...
pub(crate) struct RelaySender {
    ...
    // HYPER PATCH (relay busy-spin / battery guard): per-connection token-bucket
    // pacer. On a peer reachable ONLY via a lossy relay, noq's ConnectionDriver
    // self-wakes every poll (drive_transmit sends up to MAX_TRANSMIT_DATAGRAMS,
    // hits the cap, never marks app_limited, so CWND keeps growing → always more
    // to (re)transmit) and only parks when the socket returns Pending. The relay
    // mpsc (cap 256) drains fast so poll_send was ~always Ready → the driver spun
    // at CPU speed (observed 164% CPU + a relay writev flood, overnight battery
    // drain). This bucket returns Pending above a generous ceiling so the driver
    // PARKS; noq stashes the transmit (connection.rs buffered_transmit) and re-
    // polls on the pace timer, so it is pure backpressure — never a drop. See
    // poll_pace. The direct-path Sender (IpSender) has no pacer, so healthy P2P
    // throughput is untouched.
    tokens: f64,
    last_refill: Option<Instant>,
    pace_timer: Option<Pin<Box<Sleep>>>,
}

// HYPER PATCH: manual Clone (the pace timer `Sleep` isn't Clone). A cloned sender
// is an independent send handle, so it starts with a FRESH pacer (full bucket, no
// pending timer) exactly like create_sender — the token bucket + timer are per-
// instance transients, never shared.
impl Clone for RelaySender {
    fn clone(&self) -> Self {
        RelaySender {
...
impl RelaySender {
    ...
    pub(super) fn poll_send(
        ...
        // HYPER PATCH: pace the relay send so noq's self-waking driver parks on a
        // lossy relay-only path instead of spinning the CPU. Pending here makes noq
        // buffer the transmit and re-poll on the pace timer (never a drop).
        if self.poll_pace(cx).is_pending() {
            return Poll::Pending;
        }
        match ready!(self.sender.poll_reserve(cx)) {
            Ok(()) => {
                ...
                match self.sender.send_item(item) {
                    Ok(()) => {
                        self.tokens -= 1.0; // consume ONLY on a committed send
                        Poll::Ready(Ok(()))
                    }
    ...
    /// HYPER PATCH: token-bucket pacer for relay sends. Ready while tokens remain;
    /// otherwise arms a ~5ms timer registered on THIS ConnectionDriver's waker (cx)
    /// so the driver parks instead of busy-looping. RATE is far above any real relay
    /// use (chat/media over a relay), so it engages ONLY on the pathological spin —
    /// normal traffic sees Ready immediately and byte-for-byte unchanged behaviour.
    fn poll_pace(&mut self, cx: &mut Context) -> Poll<()> {
        const RATE: f64 = 3000.0; // datagrams/sec ceiling (~3.6 MB/s over relay)
        const BURST: f64 = 256.0; // bucket capacity (matches actor channel)
        const MIN_PACE: std::time::Duration = std::time::Duration::from_millis(5); // caps wakeups to ~200/s
        let now = Instant::now();
        ...
        let timer = self
            .pace_timer
            .get_or_insert_with(|| Box::pin(time::sleep(MIN_PACE)));
        timer.as_mut().reset(Instant::now() + MIN_PACE);
        let _ = timer.as_mut().poll(cx); // register the driver's waker
        Poll::Pending
    }
}
