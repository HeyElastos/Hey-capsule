...
/// When the next relay ping is due.
///
/// HYPER PATCH (additive, flag-gated — see [`crate::radio`]). With the flag off this is
/// plain `now + PING_INTERVAL`, identical to the `time::interval` it replaced. With the
/// flag on the deadline is deferred to the next moment the shared keepalive grid is going
/// to wake the modem anyway, so the relay ping rides along for free instead of buying its
/// own RRC transition. The wait therefore lands in `[15s, 15s + tick period)`; that is
/// still well inside the raised idle timeout, and any inbound relay message resets it here
/// exactly as before.
fn next_ping_deadline() -> Instant {
    // SERVER-DRIVEN (flag-gated — see [`crate::radio::server_driven_ping`]). The relay
    // server already pings every client every 15s + 1-5s jitter, and resets that timer on
    // every frame it receives from us. Our 15s ping is strictly shorter, so it pre-empts the
    // server's and the cadence on the wire ends up being ours. Backing off to 45s lets the
    // server drive at its own 16-20s, which is a LONGER gap for the same number of
    // exchanges — and the gap is what decides radio residency, since the modem sleeps ~10s
    // after the last packet. Ours stays armed as a backstop so a server that goes silent is
    // still noticed, just a little later.
    let period = if crate::radio::server_driven_ping() {
        crate::radio::SERVER_DRIVEN_PING_INTERVAL
    } else {
        PING_INTERVAL
    };
    if crate::radio::aligned_enabled() {
        crate::radio::snap_interval(period)
    } else {
        Instant::now() + period
    }
}
...
impl ActiveRelayActor {
    ...
    async fn run_connected(
        ...
        //
        // HYPER PATCH (additive, flag-gated — see [`crate::radio`]): this was a free-running
        // `time::interval(PING_INTERVAL)`. It is now a recomputed deadline, so that with the
        // flag on it can be snapped onto the shared tick grid — a relay ping that lands on
        // the same instant as the QUIC keepalive is free, whereas one 7s later costs a whole
        // extra radio wake. With the flag off `next_ping_deadline` is exactly `now + 15s`,
        // which is what the interval did, including its `MissedTickBehavior::Delay`
        // semantics (each deadline is measured from when the previous ping actually went
        // out, not from a fixed schedule).
        let mut ping_at = Instant::now();

        let res = loop {
            let ping_sleep = time::sleep_until(ping_at);
            n0_future::pin!(ping_sleep);
            ...
            tokio::select! {
                ...
                count = self.relay_datagrams_send.recv_many(
                    ...
                    let packet_iter = send_datagrams_buf.drain(..).map(|item| {
                        ...
                        // Instrumentation: send_relay is a BYTE count; also count the sends so a
                        // relay rate can be compared against the other transports.
                        metrics.send_relay_datagrams.inc();
