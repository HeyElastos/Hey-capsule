...
impl Addr {
    ...
    /// HYPER PATCH (additive): mirror of [`Self::is_relay`] for IP paths.
    ///
    /// The adaptive path lifecycle retires only IP paths, so it needs to say so positively
    /// rather than as "not a relay" — `Custom` is neither, and an embedder's custom
    /// transport must not be swept up by a rule written for UDP.
