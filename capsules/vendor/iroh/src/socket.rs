...
// HEY PATCH: the local address-candidate policy (see vendor/iroh/src/addr_policy.rs).
#[cfg(not(wasm_browser))]
use crate::addr_policy;
...
impl DirectAddrUpdateState {
    ...
    fn run(
        ...
        // Only ask a gateway that might exist. On cellular there is none: a CGNAT core runs
        // no UPnP/PCP responder, so the PCP + NAT-PMP + SSDP burst this fires every 20-26s is
        // pure send with zero receive (measured). The portmapper's own 5s unavailability
        // cache is shorter than the report cycle, so it never backs off on its own.
        if crate::radio::portmap_allowed() {
...
impl Actor {
        ...
        while !shutdown_token.is_cancelled() {
            ...
            tokio::select! {
                ...
                tick = self.periodic_re_stun_timer.tick() => {
                    ...
                    // Thinned while backgrounded — see radio::netreport_tick_allowed. A
                    // periodic report re-discovers a reflexive address we deliberately do not
                    // advertise under relay-collapse, and one tick can cascade into a pkarr
                    // publish plus AddAddress and a holepunch round to EVERY peer. Foreground
                    // is untouched, and non-periodic reasons (link change, dead relay) still
                    // force a report through paths this does not gate.
                    if crate::radio::netreport_tick_allowed() {
    ...
    fn handle_network_change(&mut self, is_major: bool) {
        ...
        // HYPER PATCH (additive): discard every "this address family has no route"
        // conclusion the UDP layer has reached (see `noq_udp::hyper_rearm_dead_families`).
        //
        // Those conclusions come from ENETUNREACH, i.e. from the routing table, and a link
        // change is precisely the event that rewrites it. Without this a device that moves
        // from a v4-only network onto one with working IPv6 would keep skipping IPv6 until
        // the suppression window happened to lapse — minutes of a family that works being
        // ignored, which is a connectivity bug, not a saving.
        //
        // Deliberately on BOTH major and minor changes: a "minor" change still means an
        // address or route moved, which is all it takes for a family to come back. And it
        // is the safe direction to be wrong in — the worst case is a few failed syscalls.
        //
        // This is NOT the `transports_network_change.on_network_change` hook, which runs
        // after every net_report (~25s) rather than on a link change; re-arming there would
        // cancel the suppression entirely.
        noq_udp::hyper_rearm_dead_families();
    ...
    fn update_direct_addresses(&mut self, net_report_report: Option<&net_report::Report>) {
        ...
        let stored_addrs: BTreeSet<DirectAddr> = addrs
            .into_iter()
        ...
        // ── HEY PATCH — THE SELF-REPORTING INVARIANT ─────────────────────────────────────
        // This is the ONE place every direct-address source converges before it is published:
        // portmapper, net_report QAD, the local interface sweep, and the app's own
        // `add_external_addr`. Assert here that every candidate we are about to hand to peers
        // is either globally routable, or LAN-scoped on a real (non-overlay) interface.
        //
        // This matters MORE than the filter above it. A vendored patch protects only until
        // someone re-vendors the crate — and this bug has come back that way repeatedly, silently,
        // costing days each time. An invariant protects forever, because the moment the patch is
        // gone this fires at WARN in logcat on the very first run.
        //
        // Deliberately warn-and-keep, never drop: a `qad`/`portmap`/`config` candidate is
        // externally observed, so if it looks non-global we may simply be behind a NAT the
        // observer shares — dropping it could remove a working path. Addresses are never logged,
        // only the class, the family and the interface, per the house redaction style.
        {
            let state = self.local_interfaces_watcher.get();
            for da in &stored_addrs {
                let view = iface_view(&state, da.addr.ip());
                let class = addr_policy::classify(da.addr.ip(), view);
                if class == addr_policy::AddrClass::Loopback {
                    // Upstream deliberately advertises loopback when the host has no other
                    // interface and no public address, so offline tests can still connect.
                    // Not a leak (unreachable from off-box), and warning here every 25s would
                    // devalue the WARN below — which must mean "a real regression, right now".
                    debug!("addr-policy: advertising loopback (upstream offline-test path)");
                    continue;
                }
                if !class.is_advertisable() {
                    warn!(
                        class = class.as_str(),
                        iface = view.map(|v| v.name).unwrap_or("?"),
                        prefix_len = view.map(|v| v.prefix_len).unwrap_or(0),
                        typ = %da.typ,
                        v6 = da.addr.is_ipv6(),
                        "ADDR-POLICY VIOLATION: about to advertise a candidate no peer can dial. \
                         It costs every peer a probe, a radio wake and a timeout, and an RFC1918 \
                         one can point them at an unrelated host on their own LAN. If typ=local, \
                         the vendored iroh addr_policy filter is GONE (re-vendored?) — see \
                         vendor/iroh/src/addr_policy.rs. If typ=config, the app fed it in via \
                         add_external_addr — see hey-mobile-runtime carrier::apply_advertise_policy."
                    );
                }
            }
        }
    ...
    fn collect_local_addresses(
        ...
        {
            // HEY PATCH: read the address list out of the SAME `netmon::State` snapshot we
            // attribute it against, instead of taking a second `.get()`. Two gets can straddle a
            // network change, and then every address fails attribution and the policy below sees
            ...
            // ── HEY PATCH — THE DECIDING LAYER ───────────────────────────────────────────
            // This loop is where an undialable local address actually becomes a candidate that
            // goes out to peers in QUIC NAT-traversal frames. Upstream inserts EVERY address the
            // host reports with no filtering whatsoever, which is why filtering it in the app
            // (hey-mobile-runtime::carrier) never held: this layer put it straight back.
            //
            // Measured on real hardware 2026-07-31: a Pixel on LTE advertised
            // `<carrier-NAT>/32` on `rmnet1` and `<vpn>/32` on a `tun0` whose VPN was DEAD (the
            // active default network was MOBILE via rmnet1). Neither is dialable by anyone.
            //
            // The policy KEEPS anything globally routable and any genuine LAN address on a real
            // subnet — losing a usable direct path would be a worse bug than advertising a
            // useless one, so every rule fires only on positive evidence. See addr_policy.rs.
            //
            // The loopback path above is upstream's deliberate offline-testing escape hatch and
            // is left exactly as it was.
            if !loopback_only {
                let cands: Vec<(IpAddr, Option<addr_policy::IfaceView<'_>>)> = ips
                    .iter()
                    .map(|ip| (*ip, iface_view(&netmon_state, *ip)))
                    .collect();
                // Fall back to the unfiltered set only if this is our LAST source of candidates:
                // a host whose interface metadata we misread must degrade to "advertises
                // something useless", never to "advertises nothing and is relay-only forever".
                let sel = addr_policy::select_advertisable(cands, addrs.is_empty());
                if sel.fell_back {
                    warn!(
                        count = sel.rejected.len(),
                        "addr-policy: EVERY local candidate was rejected and there is no other \
                         candidate source — restoring the unfiltered set rather than going \
                         relay-only. Interface metadata is probably being misread on this host."
                    );
                } else if !sel.rejected.is_empty() {
                    for r in &sel.rejected {
                        debug!(
                            class = r.class.as_str(),
                            iface = %r.iface,
                            v6 = r.is_v6,
                            "addr-policy: dropped a local candidate (not dialable by any peer)"
                        );
                    }
                }
                ips = sel.keep;
        ...
        //
        // HEY PATCH: deliberately NOT filtered. This is an address the caller explicitly bound
        // to, not one we swept up from the host — second-guessing it could break a deployment
        // that binds on purpose (a specific NIC, a jail, a test harness). The advertise-time
        // invariant below still reports it if it is undialable, so a mistake here is visible
...
/// HEY PATCH: recover the interface context for one of our own local addresses.
///
/// `netwatch::ip::LocalAddresses` is a bare `Vec<IpAddr>` — the interface name and the prefix
/// length are thrown away before `collect_local_addresses` ever sees them, which is why the
/// filter could not previously be written there. But the same `netmon::State` we already hold
/// for `find_flags` still carries `interfaces` (name -> addrs-with-prefix) and
/// `default_route_interface`, so the context is recoverable right here without touching any
/// dependency. That is the whole reason the fix can live at this layer.
///
/// Returns `None` when the address cannot be attributed; [`addr_policy::classify`] then judges
/// it on its IP class alone, which is the conservative (keep-it) direction.
#[cfg(not(wasm_browser))]
fn iface_view<'a>(state: &'a netmon::State, ip: IpAddr) -> Option<addr_policy::IfaceView<'a>> {
    let ip = ip.to_canonical();
    for iface in state.interfaces.values() {
        for net in iface.addrs() {
            let (addr, prefix_len) = match net {
                IpNet::V4(n) => (IpAddr::V4(n.addr()), n.prefix_len()),
                IpNet::V6 { net, .. } => (IpAddr::V6(net.addr()), net.prefix_len()),
            };
            if addr.to_canonical() == ip {
                return Some(addr_policy::IfaceView {
                    name: iface.name(),
                    prefix_len,
                    // `None` (platform could not tell us) must stay `None`: the overlay rule
                    // only fires on positive evidence that another interface owns the route.
                    is_default_route: state
...
fn new_re_stun_timer(initial_delay: bool) -> time::Interval {
    ...
    if initial_delay {
        // HYPER PATCH (additive, flag-gated — see [`crate::radio`]): snap the next re-STUN
        // onto the shared keepalive grid.
        //
        // 20-26s does not divide the keepalive period, so left alone it drifts across it
        // and buys its own radio wake roughly every time. Snapping defers it to the next
        // instant the modem is coming up anyway. The randomness is deliberately kept — it
        // exists to spread load across the relay/STUN servers, and snapping does not
        // reintroduce a fleet-wide synchronised deadline because the grid epoch is
        // per-process. The timer is rebuilt through here after every net_report, so every
        // cycle is re-snapped rather than only the first.
        if crate::radio::aligned_enabled() {
            let start = crate::radio::snap_interval(d);
            debug!("scheduling periodic_stun on the aligned grid (~{}s)", d.as_secs());
            return time::interval_at(start, d);
        }
