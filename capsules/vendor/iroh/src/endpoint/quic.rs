...
impl QuicTransportConfigBuilder {
    ...
    fn new() -> Self {
        ...
        if crate::radio::aligned_enabled() {
            // HYPER PATCH (additive, flag-gated — see `crate::radio`): hand the keepalive
            // over to the one aligned tick.
            //
            // Upstream's 5s applies to the connection AND to every path independently, and
            // 5s is shorter than the cellular RRC inactivity tail, so on its own it holds
            // the modem in RRC_CONNECTED permanently (measured: 100% residency over 6h30m
            // screen-off). `None` stops noq running its own free-running timers; the remote
            // actor pings every open path from a shared grid instead, so the several
            // periodic senders collapse onto one radio wake.
            cfg.keep_alive_interval(None);
            cfg.default_path_keep_alive_interval(None);
            // Ticking less often means a lost keepalive costs more, so buy back the margin:
            // four ticks at the connection level, three per path. Note the effective value
            // is min(ours, peer's) per RFC 9000 §10.1, so against a peer that has not been
            // upgraded this silently degrades to their 30s instead of breaking — which is
            // what makes the flag safe to roll out to one device at a time.
            cfg.default_path_max_idle_timeout(Some(crate::radio::ALIGNED_PATH_MAX_IDLE_TIMEOUT));
            match IdleTimeout::try_from(crate::radio::ALIGNED_MAX_IDLE_TIMEOUT) {
                Ok(timeout) => {
                    cfg.max_idle_timeout(Some(timeout));
                }
                Err(_) => warn!("aligned max_idle_timeout out of range, keeping the default"),
            }
        } else {
