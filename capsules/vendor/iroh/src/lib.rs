...
// HEY PATCH (additive): the local address-candidate policy + the advertise-time invariant.
// `pub` on purpose — `mod socket` is private, and hey-mobile-runtime's pinning test
// (crates/hey-mobile-runtime, `addr_policy_pins` tests) imports this module directly so the
// test fails to COMPILE if someone re-vendors iroh and drops the patch. That compile error IS
// the alarm: it is the only thing that has ever stopped this bug from silently walking back in.
#[cfg(not(wasm_browser))]
pub mod addr_policy;

// HYPER PATCH (additive): the one aligned keepalive tick that lets the cellular modem
// sleep. `pub` because the embedder has to be able to flip the flag and register the
// foreground/call posture provider. Entirely inert until `radio::set_aligned_enabled(true)`.
pub mod radio;
