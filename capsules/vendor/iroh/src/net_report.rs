...
impl Client {
    ...
    async fn spawn_qad_probes(
        ...
        if !needs_v4_probe && !needs_v6_probe {
            // HYPER PATCH: the whole point of retaining the connections — and a line
            // upstream can never reach. `needs_v4_probe` is `v4_report.is_none()`, and the
            // observer `current_v4()` consults is never written to on a stable NAT, so
            // upstream's `v4_report` is permanently `None`. See
            // `crate::radio::qad_reuse_enabled` for the full account.
            crate::radio::record_qad_cycle(0);
            return reports;
        }
        ...
        // HYPER PATCH: one QUIC+TLS handshake per spawned task, so this is the transmit
        // cost of the cycle. Recorded before the join loop because tasks are cancelled once
        // enough relays answer — the packets are already on the wire by then.
        crate::radio::record_qad_cycle(v4_buf.len() + v6_buf.len());
    ...
    fn add_report_history_and_set_preferred_relay(&mut self, r: &mut Report) {
        ...
        // HYPER PATCH: the user's OWN relay wins, when it answers.
        //
        // Everything above is a latency contest, and n0 runs a global anycast fleet, so a
        // single self-hosted VPS reliably loses it. Observed on the owner's handset: the map
        // contained his relay while the home relay was a US WEST n0 relay, selected from
        // Sweden. "My own relay" was true of the map and false of the connection.
        //
        // Latency is the wrong thing to optimise here anyway. The relay's keepalive interval
        // is compiled into the SERVER, so it can only be lowered on a relay we run — and a
        // relay 40ms further away that pings every 90s instead of every 16s is worth far
        // more to a phone than the 40ms. If we never connect to it, patching it does nothing.
        //
        // This CANNOT strand anyone: the pin is applied only when the relay produced a
        // latency in this very report, i.e. it answered us this cycle. A VPS that is down
        // has no latency, the pin is skipped, and the ordinary contest runs with n0 still in
        // the map. Failure mode is "quietly fall back to n0 until yours returns".
        if crate::radio::pinned_relay().is_some() {
            if let Some(url) = r
                .relay_latency
                .iter()
                .map(|(_, url, _)| url)
                .find(|url| crate::radio::is_pinned_relay(url.as_str()))
                .cloned()
            {
                if r.preferred_relay.as_ref() != Some(&url) {
                    debug!(%url, "pinning home relay to the user's own relay");
                }
                r.preferred_relay = Some(url);
            }
        }
...
async fn run_probe_v4(
    ...
    // HYPER PATCH: seed the observer with the report we just measured.
    //
    // `watcher` is a `tokio::sync::watch` stream, so it only ever yields again if the
    // observed address CHANGES. Upstream starts the observer at `None` and writes to it
    // only from that stream, so on a stable NAT mapping it stays `None` for the life of the
    // connection — `QadConns::current_v4` then reports "no address", `needs_v4_probe` is
    // permanently true, and every 20-26s re-STUN re-handshakes every relay in the map and
    // throws the result away. Seeding makes the connection reusable. Latency stays live
    // (`current_v4` re-reads `conn.rtt`) and a real address change still lands here through
    // the loop below.
    //
    // `qad_seed_observed` returns `None` while the flag is off, which is upstream exactly.
    let observer = Watchable::new(crate::radio::qad_seed_observed(report.clone()));
...
async fn run_probe_v6(
    ...
    // HYPER PATCH: seed the observer with the report we just measured — see the identical
    // comment in `run_probe_v4`. The v6 predicate (`v6_report.is_some() != have_v6`) is a
    // different expression from the v4 one but has the same failure mode: with the observer
    // stuck at `None` it reads "we expected v6 and have no v6 answer" forever.
    let observer = Watchable::new(crate::radio::qad_seed_observed(report.clone()));
    let endpoint = relay.url.clone();
    let handle = task::spawn(shutdown_token.run_until_cancelled_owned({
        let observer = observer.clone();
...
mod tests {
    ...
    /// HYPER PATCH: the retained QAD connection must be *reusable* after the first report.
    ///
    /// This is the defect stated as an assertion. `spawn_qad_probes` keeps the QUIC
    /// address-discovery connection open specifically so the next cycle can read our public
    /// address off it — `needs_v4_probe` is literally `current_v4().is_none()`. Upstream
    /// builds the report from the first observed address and then seeds the connection's
    /// observer with `None`, and the only writer to that observer is a `watch` stream that
    /// fires on change only. A stable NAT mapping never changes, so `current_v4()` answers
    /// `None` for the life of the connection and every cycle re-handshakes every relay.
    ///
    /// Run against a real (local) relay, so this exercises the actual handshake, the actual
    /// `OBSERVED_ADDRESS` frame and the actual retention path — not a model of them.
    ///
    /// The interface state is deliberately v4-only: the test relay listens on 127.0.0.1, so
    /// claiming v6 would leave `needs_v6_probe` true forever for reasons that have nothing
    /// to do with what is being tested.
    #[tokio::test(flavor = "multi_thread")]
    #[traced_test]
    async fn qad_conn_is_reusable_after_the_first_report() -> Result<()> {
        let (server, relay) = test_utils::relay().await;
        let client_config = iroh_relay::tls::make_dangerous_client_config();
        let ep = noq::Endpoint::client(SocketAddr::new(Ipv4Addr::LOCALHOST.into(), 0)).anyerr()?;
        let quic_addr_disc = QuicConfig {
        ...
        // ---- disarmed: upstream behaviour, byte for byte ----
        crate::radio::set_qad_reuse_enabled(false);
        let mut client = Client::new(
            resolver.clone(),
            relay_map.clone(),
            opts.clone(),
        ...
        let r = client
            .get_report(if_state.clone(), false, cancel.child_token())
            .await;
        assert!(r.global_v4.is_some(), "the probe must have learned an address");
        assert!(
            client.qad_conns.v4.is_some(),
            "the connection is retained either way — that is not the bug"
        );
        assert!(
            client.qad_conns.current_v4().is_none(),
            "upstream throws the measured address away, so the retained connection reports \
             nothing and `needs_v4_probe` stays true forever"
        );
        cancel.cancel();
        drop(client);

        // ---- armed: the retained connection answers, so the next cycle is free ----
        crate::radio::set_qad_reuse_enabled(true);
        let mut client = Client::new(resolver, relay_map, opts, Default::default());
        let cancel = CancellationToken::new();
        let r = client
            .get_report(if_state.clone(), false, cancel.child_token())
            .await;
        assert!(r.global_v4.is_some(), "the probe must still learn an address");
        let cached = client
            .qad_conns
            .current_v4()
            .expect("the retained connection must now carry the address we measured");
        assert!(
            matches!(cached, ProbeReport::QadIpv4(_)),
            "and it must come back as a v4 QAD report"
        );

        // The consequence, stated in the terms `spawn_qad_probes` actually uses. With
        // `have_v6: false` and a cached v4 report, `needs_v4_probe` (`is_none()`) and
        // `needs_v6_probe` (`is_some() != have_v6`) are BOTH false, which is the
        // early-return that spawns nothing. Asserted as state rather than as a global
        // counter delta on purpose: the counters are process-wide and this binary runs its
        // tests concurrently, so a delta assertion here would be a flake, not a proof.
        assert!(
            client.qad_conns.current_v6().is_none(),
            "no v6 connection was asked for, so none may exist"
        );

        // Corroboration through the same counter the on-device readout prints. Only a cycle
        // that spawned zero probes increments it, and the cycle below is one; concurrent
        // tests can only push it further up, never down, so `>=` is the honest form.
        let (_, _, before) = crate::radio::qad_counters();
        let r2 = client
            .get_report(if_state.clone(), false, cancel.child_token())
            .await;
        let (_, _, after) = crate::radio::qad_counters();
        assert!(
            after >= before + 1,
            "the incremental cycle must be served from the retained connection"
        );
        assert!(
            r2.global_v4.is_some(),
            "and it must still report our address — reuse may not cost knowledge"
        );
        assert!(
            client.qad_conns.current_v4().is_some(),
            "the connection must still be reusable for the cycle after that"
        );
        cancel.cancel();
