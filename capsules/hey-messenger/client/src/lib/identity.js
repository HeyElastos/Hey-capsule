...
// ── Post-quantum and X25519 derivation ─────────────────────────────
//
// Hey Messenger derives THREE keypairs from the same 32-byte authKey:
//   - Ed25519 (sign/verify events)
//   - X25519  (classical leg of hybrid E2E)  — same seed, different curve math
//   - ML-KEM-768 (post-quantum leg)         — HKDF-expanded to 64 bytes
//
// Deterministic derivation keeps recovery simple: the authKey alone is
// enough to restore every key. The ML-KEM expansion uses an HKDF
// info-string that's distinct from anything else we derive from the same
// seed, so the keys are domain-separated.

const ML_KEM_INFO = new TextEncoder().encode("hey-messenger/ml-kem-768-v1");

// X25519 pubkey from the same 32-byte seed used for Ed25519. The two
// pubkeys are unrelated mathematically (different curve) — sharing the
// seed doesn't weaken either.
...
// ML-KEM-768 keypair deterministically derived from the authKey seed.
// HKDF expands the 32B seed into the 64B (d || z) seed ML-KEM expects.
...
// One-shot bundle: every keypair derivable from the authKey, plus the
// DID. Used at sign-in time to fill the session cache.
export const expandFullIdentity = (authKeyHex) => {
  const { seed, publicKey } = keypairFromAuthKey(authKeyHex);
  const didKey = publicKeyToDidKey(publicKey);
  const xkp = x25519FromSeed(seed);
  const kkp = mlKemFromSeed(seed);
  return {
    didKey,
    ed: { seed, publicKey },
    x25519: xkp,
