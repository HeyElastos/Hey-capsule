...
- **The gateway provider proxy has a SEPARATE allowlist from the redemption endpoint.** This is the gate that makes a 401 → 403 with Hey today. Confirmed by dev 2026-05-29: it's actually narrower than the wider screenshot suggested — `gateway.rs:1156` only accepts `documents`, sometimes `library`. YNH-fork patch 0001 opens `/runtime-token` (so we can mint a bearer) but does NOT add hey-social to the provider proxy allowlist. Net: Hey can authenticate; every Carrier / content / DID call still 403s. **The right fix is manifest/capability-based, not allowlist extension** — make the allowlist check capability tokens, not hardcoded app names. Cannot be bypassed in-capsule.

- **`GET /api/session` returns SESSION METADATA, NOT identity.** Body is `{session_id, session_type, vm_id, capabilities_count, created_at, last_active}` (source: `handlers/capability.rs:527-572`, confirmed dev 2026-05-29). No `did`, `didKey`, `principal`, or `identity.did` field. `inherit_session` will always return None on stock upstream — Landing falls back to the passkey ceremony (the "transitional empty auth_key_hex + lazy passkey" path). Until a real identity surface exists (extended /api/session OR `elastos://did/sign` provider), inherit-from-session is a no-op.

- **The right auth flow is three-step, not one.** (1) Home launch token → app bearer/session via redemption endpoint. (2) App bearer/session → request granted capabilities. (3) Provider call carries BOTH the bearer/cookie AND the per-capability token (`X-Capability-Token`, source: `handlers/provider.rs:31`). Launch token alone does NOT grant ambient provider access; bearer alone does NOT pass the provider proxy. Anyone who confuses the layers gets a 401 fixed only to discover 403 next.
