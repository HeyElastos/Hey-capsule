...
- Express.js
- JWT Authentication
- File-based JSON persistence for lightweight demos


## Schema Diagram

![Schema Diagram](https://via.placeholder.com/800x400?text=Hey+Schema+Diagram)
...
If you prefer, the Flask classifier server can still be run independently from the `classifier_server` directory.

## Usage

This version of Hey includes a lightweight React frontend and a minimal Express backend. The backend exposes:

- `POST /api/users/signup` — create a nickname-based account and receive a generated Hey key
- `POST /api/users/signin` — authenticate with the generated key
